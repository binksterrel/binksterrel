...
<div style={{ position: 'relative', display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: '#06020a', borderRadius: 16, overflow: 'hidden', fontFamily: 'monospace', border: '1px solid rgba(251,38,65,0.18)' }}>
  <style>{`
    @keyframes tm-orb-a { 0%, 100% { transform: translate(0,0); opacity: 0.5; } 50% { transform: translate(20px,-14px); opacity: 0.8; } }
    @keyframes tm-orb-b { 0%, 100% { transform: translate(0,0); opacity: 0.4; } 50% { transform: translate(-16px,12px); opacity: 0.7; } }
    @keyframes tm-cur { 0%, 49% { opacity: 1; } 50%, 99% { opacity: 0; } }
    @keyframes tm-l1 { 0%,100%{opacity:0} 8%,95%{opacity:1} }
    @keyframes tm-l2 { 0%,100%{opacity:0} 18%,95%{opacity:1} }
    @keyframes tm-l3 { 0%,100%{opacity:0} 28%,95%{opacity:1} }
    @keyframes tm-l4 { 0%,100%{opacity:0} 38%,95%{opacity:1} }
    @keyframes tm-l5 { 0%,100%{opacity:0} 48%,95%{opacity:1} }
    @keyframes tm-l6 { 0%,100%{opacity:0} 58%,95%{opacity:1} }
    @keyframes tm-l7 { 0%,100%{opacity:0} 68%,95%{opacity:1} }
    @keyframes tm-l8 { 0%,100%{opacity:0} 78%,95%{opacity:1} }
    #tmo1 { animation: tm-orb-a 9s ease-in-out infinite; }
    #tmo2 { animation: tm-orb-b 11s ease-in-out infinite 1s; }
    #tmo3 { animation: tm-orb-a 8s ease-in-out infinite 2s; }
  ...
  <svg width="800" height="200" style={{ position: 'absolute', top: 0, left: 0 }}>
    <defs>
      <radialGradient id="tmg1" cx="50%" cy="50%" r="50%">
        <stop offset="0%" stopColor="rgba(251,38,65,0.45)" />
        <stop offset="100%" stopColor="rgba(251,38,65,0)" />
      </radialGradient>
      <radialGradient id="tmg2" cx="50%" cy="50%" r="50%">
        <stop offset="0%" stopColor="rgba(253,177,73,0.35)" />
        <stop offset="100%" stopColor="rgba(253,177,73,0)" />
      </radialGradient>
      <radialGradient id="tmg3" cx="50%" cy="50%" r="50%">
        <stop offset="0%" stopColor="rgba(254,149,105,0.3)" />
        <stop offset="100%" stopColor="rgba(254,149,105,0)" />
      </radialGradient>
    </defs>
    <ellipse id="tmo1" cx="680" cy="160" rx="180" ry="140" fill="url(#tmg1)" />
    <ellipse id="tmo2" cx="750" cy="50"  rx="150" ry="120" fill="url(#tmg2)" />
    <ellipse id="tmo3" cx="600" cy="180" rx="130" ry="110" fill="url(#tmg3)" />
  </svg>
  <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', padding: '20px 28px', gap: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16 }}>
      <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#fb2641' }} />
      <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#fe9569' }} />
      <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#fdb149' }} />
      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginLeft: 10, letterSpacing: 2 }}>terrel@macbook ~ %</span>
    </div>
    <div id="tl1" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: '#fb2641', fontSize: 13 }}>$</span>
      <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{'git commit -m "feat: pipeline SharePoint LLM @ EDF"'}</span>
    </div>
    <div id="tl2" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13 }}>{'→'}</span>
      <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 13 }}>{'2 200+ entrees traitees · branch main'}</span>
    </div>
    <div id="tl3" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: '#fb2641', fontSize: 13 }}>$</span>
      <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{'python train.py --model random_forest --dataset banquise'}</span>
    </div>
    <div id="tl4" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13 }}>{'→'}</span>
      <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 13 }}>{'accuracy: 0.94 · f1: 0.91 · saved to /models'}</span>
    </div>
    <div id="tl5" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: '#fb2641', fontSize: 13 }}>$</span>
      <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{'npx vercel deploy --prod'}</span>
    </div>
    <div id="tl6" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13 }}>{'→'}</span>
      <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 13 }}>{'portfolio-terrel.vercel.app · 1.2s'}</span>
    </div>
    <div id="tl7" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: '#fb2641', fontSize: 13 }}>$</span>
      <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{'nx build episteme --graph --algo louvain'}</span>
    </div>
    <div id="tl8" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
      <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13 }}>{'→'}</span>
      <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 13 }}>{'1 703 nodes · 12 communities · pagerank done'}</span>
    </div>
    <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
      <span style={{ color: '#fb2641', fontSize: 13 }}>$</span>
      <span id="tm-cur" style={{ color: '#fb2641', fontSize: 13 }}>{'▋'}</span>
    </div>
  </div>
</div>
```
